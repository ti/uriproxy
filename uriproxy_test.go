...
func TestReverseProxy(t *testing.T) {
	...
	p := &Proxy{
		Routers: []Router{{
			Pattern: "/",
			Handler: newFakeUpstream(backend.URL, false),
		}},
	}
...
func TestReverseProxyInsecureSkipVerify(t *testing.T) {
	...
	p := &Proxy{
		Routers: []Router{{
			Pattern: "/",
			Handler: newFakeUpstream(backend.URL, true),
		}},
	}
...
func TestReverseProxyMaxConnLimit(t *testing.T) {
	...
	su, err := NewStaticUpstream(backend.URL + "?max_conns=" + fmt.Sprint(MaxTestConns))
	if err != nil {
		t.Fatal(err)
	}
	...
	for i := 0; i < MaxTestConns; i++ {
		...
		go func(i int) {
			defer jobs.Done()
			w := httptest.NewRecorder()
			code, err := p.DoProxy(w, httptest.NewRequest("GET", "/", nil),nil)
			if err != nil {
...
func TestWebSocketReverseProxyNonHijackerPanic(t *testing.T) {
	...
	defer func() {
		r := recover()
		if !strings.Contains(fmt.Sprint(r), "is not a hijacker") {
...
func newMultiHostTestProxy() *Proxy {
	...
	p := &Proxy{
		Routers: []Router{{
			Pattern: "/",
			Handler: &fakeUpstream{name: upstreamServer1.URL,},
			},
			{
				Pattern: "/api",
				Handler: &fakeUpstream{name: upstreamServer2.URL,},
			}},
	}
...
func TestBasicAuth(t *testing.T) {
	...
	basicAuthTestcase(t, url.UserPassword("unused", "unused"), url.UserPassword("username", "password"))
}
...
func basicAuthTestcase(t *testing.T, upstreamUser, clientUser *url.Userinfo) {
	...
	p := &Proxy{
		Routers: []Router{{
			Pattern: "/",
			Handler: newFakeUpstream(backURL.String(), false),
		},},
	}
	...
	if clientUser != nil {
		if upstreamUser == nil {
			if string(body) != clientUser.String() {
				t.Fatalf("Invalid auth info: %s but expect %s", string(body), clientUser.String())
			}
		} else {
			if string(body) != upstreamUser.String() {
				t.Fatalf("Invalid auth info: %s but expect %s", string(body), upstreamUser.String())
			}
		}
	} else {
		if upstreamUser != nil {
...
func TestReverseProxyRetry(t *testing.T) {
	...
	backUri, _ := url.Parse(backend.URL)

	su, err := NewStaticUpstream("http://localhost:65535,localhost:65534," + backUri.Host + backUri.Path + "?policy=round_robin&fail_timeout=5s&max_fails=1&try_duration=5s&try_interval=250ms")
	if err != nil {
		t.Fatal(err)
	}
...
func TestReverseProxyLargeBody(t *testing.T) {
	...
	su, err := NewStaticUpstream(backend.URL)
	if err != nil {
		t.Fatal(err)
	}
...
func TestCancelRequest(t *testing.T) {
	...
	status, err := p.DoProxy(rec, req,nil)
...
func newPrefixedWebSocketTestProxy(backendAddr string, prefix string) *Proxy {
	return &Proxy{
		Routers: []Router{{
			Pattern: "/",
			Handler: &fakeWsUpstream{name: backendAddr, without: prefix},
		},},
	}
}
