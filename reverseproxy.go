...
func (rp *ReverseProxy) ServeHTTP(rw http.ResponseWriter, outreq *http.Request, respUpdateFns []RespUpdateFn) error {
	...
	for _, respUpdateFn := range respUpdateFns {
		isEnd := respUpdateFn(res)
		if isEnd  {
			return nil
		}
	}
	...
	if isWebsocket {
		...
		if !ok {
			panic(fmt.Sprintf("%T is not a hijacker", rw))
		}
...
//RespUpdateFn
//return isEnd, if is close it will no continue, you can do get some custom to write
type RespUpdateFn func(resp *http.Response) (isEnd bool)
